...
- [x] 1. `lib/notion/queries.ts`에 `searchResearches` 함수 추가
...
- [x] 종목명 검색 → 해당 종목 포함 결과만 노출
- [x] 태그 키워드 검색 → 태그 기반 필터링 결과 노출
- [x] 입력 후 300ms 동안 API 요청 발생하지 않음 (네트워크 탭 확인)
- [x] 검색 후 최근 검색어 목록에 추가됨
- [x] 결과 없음 상태 UI 표시
- [x] 빈 쿼리 요청 시 빈 배열 반환 (API)
