...
**목적**: Claude가 생성한 종목 분석 리서치를 Notion에 축적하고, 동일 종목의 투자의견/목표가 변화를 시계열로 추적한다.
**사용자**: 프로젝트 소유자 1인 — Claude로 종목 분석을 요청하고 결과를 개인 투자 학습 기록으로 보관한다.

---

## 사용자 스토리

### 분석 요청자 (Claude 사용자) 관점
...
3. [웹 - 메인 페이지]
   Cache Components / ISR 로 발행된 리서치 반영
   카드 그리드로 최신 리서치 열람
   ↓
...
4. 분기점
   A. 카드 클릭 → [상세 페이지] → Notion 본문 렌더링 + 동일 종목 이전 리서치 링크
   B. 종목명 클릭 / 직접 접근 → [종목 히스토리 페이지] → 시계열 목표가·투자의견 차트 + 리서치 목록
...
```
AI Stock Research Archive 내비게이션

헤더 메뉴 (전체 공개)
├── 홈 (메인 페이지) - F001, F002
├── 검색 (검색 페이지) - F003
└── [종목명] (종목 히스토리 페이지) - F005  ← 카드 또는 상세에서 링크 진입

페이지 내 이동
├── 리서치 카드 클릭 → 상세 페이지 - F004
...
> **구현 기능:** `F001`, `F002`, `F011` | **접근:** 헤더 홈 메뉴 또는 루트 경로

| 항목 | 내용 |
|------|------|
| **역할** | 발행된 모든 리서치의 진입점. 최신 분석을 카드 그리드로 한눈에 파악 |
| **진입 경로** | 헤더 홈 링크 클릭, 사이트 루트 접근, 다른 페이지에서 로고 클릭 |
| **사용자 행동** | 카드 목록을 훑어보고 관심 리서치를 클릭하거나, 섹터·태그 필터로 범위를 좁혀 탐색 |
| **주요 기능** | • 발행 상태 리서치 카드 그리드 (종목명, 투자의견 배지, 목표가+통화단위, 발행일, 요약 표시)<br>• 섹터 필터 드롭다운 (클라이언트 필터링)<br>• 태그 멀티 필터 (클라이언트 필터링, 다중 선택 시 OR 조건)<br>• 페이지네이션 (페이지당 20건, Notion API cursor 기반)<br>• 면책 문구 푸터<br>• **리서치 카드 클릭** → 상세 페이지 이동 |
| **다음 이동** | 카드 클릭 → 상세 페이지 / 종목명 클릭 → 종목 히스토리 페이지 / 검색창 입력 → 검색 페이지 |
...
| 항목 | 내용 |
|------|------|
| **역할** | 동일 종목의 모든 리서치를 시계열로 보여주고, 목표가·투자의견 변화를 시각화. 프로젝트의 차별 핵심 화면 |
| **진입 경로** | 메인 페이지 종목명 클릭, 상세 페이지 "이 종목의 모든 리서치 보기" 링크, 직접 URL 접근 |
| **사용자 행동** | 목표가 추이 차트 확인, 투자의견 변화 타임라인 확인, 각 시점 분석 요약 비교, 특정 시점 상세 페이지로 이동 |
| **주요 기능** | • 종목 기본 정보 헤더 (종목명, 티커, 섹터)<br>• 목표가 변화 line chart (recharts, x축: publishedAt, y축: targetPrice, 단일 통화만 렌더링 — 혼재 시 경고 메시지)<br>• 투자의견 변화 타임라인 (날짜별 매수/관망/매도 배지)<br>• 리서치 시계열 목록 (날짜, 투자의견, 목표가+통화단위, 요약, AI 모델명, 상세 링크)<br>• 차트 데이터 접근성용 `<table>` 병기<br>• 면책 문구 푸터 |
...
| `target_price` | Number | 목표가 (숫자만, 통화는 currency 필드 참조) |
...
| `ai_model` | Rich Text | 사용한 Claude 모델명 (예: `Claude Opus 4.7`) |
...
| target_price | 숫자만 (통화 단위는 currency 필드에 별도 저장) |
| currency | `KRW` 또는 `USD` |
| summary | 50자 이내 한 줄 요약 |
| published_at | 분석 수행 날짜 (ISO 8601 형식) |
| status | 반드시 `draft` 로 저장 (발행은 사용자가 직접 변경) |
| ai_model | 사용한 Claude 모델명 정확히 기재 (예: `Claude Opus 4.7`) |
